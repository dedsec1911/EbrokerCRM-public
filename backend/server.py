...
async def register(user_data: UserRegister):
    # Only allow agent registration through public endpoint
...
async def register_admin(user_data: UserRegister):
    """
    Admin registration endpoint - Only allows creating ONE admin.
    Checks if admin already exists, if not, creates first admin.
    """
...
async def generate_whatsapp_message(request: WhatsAppMessageRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Build a concise message with link to property page (images available there)
    message_parts = [
        f"🏢 PROPERTY DETAILS",
        f"",
    ...
    # Use quote_plus which handles emojis and spaces properly
